...
da = eng[eng['Tag'] == 1].iloc[:samples]  #Default value. You can check for other values by below lines.
...
db = eng[eng['Tag']==0].iloc[:samples]    #Default value. You can check for other values by below lines.
...
# Select rows with the specified label
dk = hind[hind['Tag'] == 1].iloc[:samples]  #Default value. You can check for other values by below lines.
...
dm = hind[hind['Tag']==0].iloc[:samples]     #Default value. You can check for other values by below lines.
...
best_model_params_path = os.path.join(tempdir, f"best_model_indicbert(cm_ratio)_Com.pt")
...
with open('/data1/aakash/Codemix/Aakash_02/bash/output.txt', 'a') as f:
    f.write(f"0indicBERT_without_Trans_Combined_exp2_200_{samples}.py \n")
