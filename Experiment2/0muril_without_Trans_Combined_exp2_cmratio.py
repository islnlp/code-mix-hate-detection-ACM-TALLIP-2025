...
parser.add_argument('--samples', type=int, default=200, help='200, 400, 600, 800, 1000, 1200, 1400')
...
# select how many samples to augment (Exp2)
samples = args.samples

# Calculate the ratio of each label
label_counts = train_df['Tag'].value_counts(normalize=True)
ratio_0 = label_counts[0]
ratio_1 = label_counts[1]

# Select rows with the specified label
# Calculate the number of samples for each label based on the ratio
num_samples_0 = int(samples * ratio_0)
num_samples_1 = samples - num_samples_0  # Ensuring the total number is 'samples'

da = eng[eng['Tag'] == 1].iloc[:num_samples_1]    #Default value. You can check for other values by below lines.
#da = eng[eng['Tag'] == 1].iloc[:146]
#da = eng[eng['Tag'] == 1].iloc[:218]
#da = eng[eng['Tag'] == 1].iloc[:291]
#da = eng[eng['Tag'] == 1].iloc[:364]
#da = eng[eng['Tag'] == 1].iloc[:436]
#da = eng[eng['Tag'] == 1].iloc[:509]
da = da.reset_index(drop=True)

db = eng[eng['Tag'] == 0].iloc[:num_samples_0]    #Default value. You can check for other values by below lines.
#db = eng[eng['Tag'] == 0].iloc[:254]
#db = eng[eng['Tag'] == 0].iloc[:382]
#db = eng[eng['Tag'] == 0].iloc[:509]
#db = eng[eng['Tag'] == 0].iloc[:636]
#db = eng[eng['Tag'] == 0].iloc[:764]
#da = eng[eng['Tag'] == 0].iloc[:891]
...
dk = hind[hind['Tag'] == 1].iloc[:num_samples_1]    #Default value. You can check for other values by below lines.
#dk = hind[hind['Tag'] == 1].iloc[:146]
#dk = hind[hind['Tag'] == 1].iloc[:218]
#dk = hind[hind['Tag'] == 1].iloc[:291]
#dk = hind[hind['Tag'] == 1].iloc[:364]
#dk = hind[hind['Tag'] == 1].iloc[:436]
#dk = hind[hind['Tag'] == 1].iloc[:509]
dk = dk.reset_index(drop=True)

dm = hind[hind['Tag'] == 0].iloc[:num_samples_0]    #Default value. You can check for other values by below lines.
#dm = hind[hind['Tag'] == 0].iloc[:254]
#dm = hind[hind['Tag'] == 0].iloc[:382]
#dm = hind[hind['Tag'] == 0].iloc[:509]
#dm = hind[hind['Tag'] == 0].iloc[:636]
#dm = hind[hind['Tag'] == 0].iloc[:764]
#dk = hind[hind['Tag'] == 0].iloc[:891]
...
best_model_params_path = os.path.join(tempdir, f"best_model_muril_cm_ratio_Com.pt")
...
with open('/data1/aakash/Codemix/Aakash_02/bash/output.txt', 'a') as f:
    f.write(f"0muril_without_Trans_Combined_exp2_cmratio_{samples}.py \n")
...
# output_filename = 'predicted_labels_combined(mbert_without_trans_cmratio).csv'
