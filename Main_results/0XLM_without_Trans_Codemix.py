...
logging.basicConfig(filename=f'./logs/xlm(Cod)_{time.asctime().replace(" ","_")}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
Cod = pd.read_csv('/data1/aakash/Codemix/Aakash_02/1HateSpeech_Codemix.csv')
Cod = Cod.dropna()
...
best_model_params_path = os.path.join(tempdir, f"model_XLM_Codemix.pt")
...
# output_filename = 'predicted_labels_codemix(XLM_without_Trans).csv'
