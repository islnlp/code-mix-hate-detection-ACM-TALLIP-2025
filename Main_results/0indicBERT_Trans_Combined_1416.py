...
logging.basicConfig(filename=f'/data1/aakash/Codemix/Aakash_02/logs/mBERTCo_{time.asctime().replace(" ","_")}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
# Training
# weightage for weighted loss
label_counts = t_com['Tag'].value_counts()
total_count = len(t_com)
...
best_model_params_path = os.path.join(tempdir, f"best_model_indicbert1_Com.pt")
...
for epoch in range(epochs):
    ...
    c = 249
    d = 438
    val_weightage=torch.tensor([c/(c+d),d/(c+d)]).to(device)
